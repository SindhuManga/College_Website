pipeline {
    ...
    environment {
        ...
        // Define the ECR image URL
        ECR_IMAGE_URL = "${ECR_REPO}:latest"
        
        // Define the deployment script once in the environment block (Cleaner syntax)
        DEPLOY_SCRIPT = """
    ...
    stages {
        ...
        stage('Deploy to EC2 (via SSM)') {
            steps {
                ...
                withCredentials([usernamePassword(credentialsId: 'aws-ecr-creds', usernameVariable: 'AWS_ACCESS_KEY_ID', passwordVariable: 'AWS_SECRET_ACCESS_KEY')]) {
                    // Send the command using the DEPLOY_SCRIPT variable from the environment block
                    bat """
